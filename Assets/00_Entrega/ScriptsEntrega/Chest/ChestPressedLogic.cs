...
public sealed class ChestPressedLogic : MonoBehaviour
{
    ...
    public void OnChestPressed()
    {
        ...
        if (item == null)
        {
            Debug.LogWarning($"[Chest] Rareza {r} seleccionada, pero no hay ítems de esa rareza en el catálogo de código.");
            return;
        }

        Debug.Log($"[Chest] Premio: {item.name}  (Rareza: {r})");
        // TODO: acá instanciar/añadir a inventario si querés
    }
